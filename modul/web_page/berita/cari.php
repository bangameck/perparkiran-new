...
$cari = $_GET['key'];
    ...
    $key = $db->real_escape_string(htmlentities(trim(filter_input(INPUT_POST,'berita',FILTER_SANITIZE_STRING))));
var_dump($_GET['key']);
    die();
    if(strlen($key)<3) {
        echo '
...
<section id="content">
    <div class="content-wrap">
        ...
        $data1 = $db->query("SELECT * FROM blog WHERE publish='Y' AND j_blog LIKE %$cari%");
