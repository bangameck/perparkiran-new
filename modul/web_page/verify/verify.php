...
if ($v == true) {
    if (date('Y-m-d H:i:s', strtotime($v['batas_waktu'])) > date('Y-m-d H:i:s')) {
        ...
        sweetAlert('', 'verify-error', 'Waktu Berakhir !', 'Verifikasi Gagal, Waktu Verifikasi Kadaluawarsa, Batas Verfikasi ' . date('Y-m-d H:i:s', strtotime($v['batas_waktu'])) . '. Waktu Saat ini ' . date('Y-m-d H:i:s') . '. Silahkan Registrasi ulang, Terimakasih.');
    }
} else {
    sweetAlert('', 'verify-error', 'Kode Tidak Ditemukan !', 'Verifikasi Gagal, kode tidak ditemukan, Kemungkinan anda sudah pernah memverifikasi data sebelumnya atau Silahkan Menghubungi admin untuk info lebih lanjut.');
}
